...
public class EmployeeSyncLXV7 {
    ...
    public static void sync() throws IOException, InterruptedException {
        ...
        String path = "/root/aiminding/exam_fail_userId.txt";
        ...
        userIdList.stream().forEach(staffId -> executorService.submit(() -> {
            JSONObject req = new JSONObject();
            req.put("staff_id", staffId);
            req.put("name", "agency" + staffId);
            try {
                Boolean res = createUser(req.toJSONString());
                if (res) {
        ...
        // 将同步失败的userId写到文件中
        BufferedWriter writer = new BufferedWriter(new FileWriter("/root/aiminding/failedUserId.txt"));
    ...
    private static boolean createUser(String param) {
        ...
        if (SUCCESS_CODE.equals(code) || USER_EXIST_CODE.equals(code)) {
            if (USER_EXIST_CODE.equals(code)) {
                JSONObject jsonObject = JSONObject.parseObject(param);
                System.out.println("已经存在的员工id:" + jsonObject.getString("staff_id"));
            }
            return true;
        }
        return false;
